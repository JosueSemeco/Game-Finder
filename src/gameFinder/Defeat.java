...
public class Defeat extends JPanel implements ActionListener{
	
	int anchoVentana = Main.frame.getWidth(),
			altoVentana = Main.frame.getHeight();
	
	public static JFrame def;
...
// BOTON X
		...
		JLabel error = new JLabel("OH NO!");
		error.setBounds(135, 50, 400, 100);
		...
		JLabel valido = new JLabel("HAS PERDIDO...");
		valido.setBounds(160, 180, 545, 45);
		valido.setFont(loadPixelFont("Hardpixel.OTF", 45));
		valido.setForeground(new Color(255, 255, 240));
		this.add(valido);
		
		JLabel volviendo = new JLabel("VOLVIENDO AL MENU PRINCIPAL");
		volviendo.setBounds(40, 275, 545, 35);
		volviendo.setFont(loadPixelFont("Hardpixel.OTF", 35));
		volviendo.setForeground(new Color(255, 255, 240));
		this.add(volviendo);
